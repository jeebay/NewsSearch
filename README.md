...
Figuring out the best way to share state between a two views was tricky. My instinct was that filter data should be in a singleton but that doesn't appear to be the Android way.

Instead, I serialized a filter model and update a private variable in each activity based on the model that gets passed back and forth.
